...
export function useSidebarResize({
	...
	React.useEffect(() => {
		const handleMouseMove = (e: MouseEvent) => {
			...
			// For right sidebar, invert the direction
			const effectiveDeltaX = side === "right" ? -deltaX : deltaX;
			const absDeltaX = Math.abs(deltaX);
			if (!isDragging.current && absDeltaX > 5) {
			...
			if (isDragging.current) {
				...
				const newWidthPx = startWidth.current + effectiveDeltaX;

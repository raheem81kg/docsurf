   ...
   return (
      <div className="mb-2 flex h-auto animate-[fade-in-up] flex-col space-y-2 rounded-lg border border-border-subtle bg-bg-subtle px-3 py-2.5 font-semibold text-sm shadow-elevation-low md:max-w-sm">
         <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => onClose(toastId)}>
               <X className="h-4 w-4 hover:cursor-pointer" />
            </button>
         </div>
         <Progress value={progress} className="h-2" />
         <div className="text-right font-normal text-xs">{Math.floor(progress)}%</div>
      </div>
   );
};
...
export function showProgressToast(progress: number, message?: string, toastId = "download-progress") {
   ...
   return toast.custom((id) => <ProgressToast message={message} progress={progress} onClose={onClose} toastId={id} />, {
      id: toastId,

...
export const ErrorToast = ({
	...
	>
		...
		<span className="mt-0.5">
			<X
				className="h-4 w-4 text-semantic-error hover:cursor-pointer"
				onClick={() => onClose(toastId)}
			/>
		</span>
	</div>
...
export const WarningToast = ({
	...
	>
		...
		<span className="mt-0.5">
			<X
				className="h-4 w-4 text-semantic-attention hover:cursor-pointer"
				onClick={() => onClose(toastId)}
			/>
		</span>
	</div>
...
export function showToast(
	...
	options:
		| number
		| (ExternalToast & { action?: ToastAction }) = TOAST_VISIBLE_DURATION,
) {
	let _options: ExternalToast & { action?: ToastAction } =
	...
	const action =
		typeof options === "object" && "action" in options
			? options.action
			: undefined;

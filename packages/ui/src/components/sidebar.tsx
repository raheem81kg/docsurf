...
import { Sheet, SheetContent, SheetHeader, SheetTitle } from "@docsurf/ui/components/sheet";
...
import { useSidebarResize } from "@docsurf/ui/hooks/use-sidebar-resize";
      ...
      return (
         <Sheet
            ...
            onOpenChange={(nextOpen) => {
               // this is how we keep in sync the sidebar state with sandbox while on mobile
               setOpenMobile(nextOpen);
               if (typeof set_l_sidebar_state === "function" && side === "left") {
                  set_l_sidebar_state(nextOpen);
               }
               if (typeof set_r_sidebar_state === "function" && side === "right") {
                  set_r_sidebar_state(nextOpen);
               }
               if (typeof set_ir_sidebar_state === "function" && side === "right") {
                  set_ir_sidebar_state(nextOpen);
               }
            }}
            {...props}
         >
...
function SidebarRail({
   ...
   enableDrag?: boolean;
   onToggle?: () => void;
   sideForDrag?: "left" | "right";
   maxSidebarWidth?: number;
}) {
   if (enableDrag && !sideForDrag) {
      throw new Error("SidebarRail: 'sideForDrag' prop is required when 'enableDrag' is true.");
   }
   const { toggleSidebar, width, setWidth, state, setIsDraggingRail } = useSidebar();
   const { dragRef, handleMouseDown } = useSidebarResize({
      enableDrag,
      onResize: setWidth,
      onToggle: onToggle || toggleSidebar,
      currentWidth: width,
   ...
   return (
      <button
         ...
         className={cn(
            ...
            enableDrag && "[[data-side=left]_&]:cursor-col-resize [[data-side=right]_&]:cursor-col-resize",
            className
         )}
         {...props}
      />

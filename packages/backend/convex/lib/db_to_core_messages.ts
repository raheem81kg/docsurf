...
import { getFileTypeInfo, isImageMimeType } from "@docsurf/utils/constants/file_constants";
...
export const dbMessagesToCore = async (messages: Infer<typeof Message>[], modelAbilities: ModelAbility[]): Promise<CoreMessage[]> => {
               ...
               if (fileTypeInfo.isImage && isImageMimeType(p.mimeType || "")) {
                  ...
                  try {
                     ...
                     if (data.ok) {
                        ...
                        console.warn(`[cvx][chat] Failed to fetch image file ${p.data}: ${data.status} ${data.statusText}`);
                        failedFileFetch("image", filename);
                     }
                  } catch (error) {
                     console.warn(`[cvx][chat] Error processing image file ${p.data}:`, error);
                     failedFileFetch("image", filename);
                  }
                  ...
                  try {
                     ...
                     if (data.ok) {
                        ...
                        console.warn(`[cvx][chat] Failed to fetch text file ${p.data}: ${data.status} ${data.statusText}`);

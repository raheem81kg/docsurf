...
async function getCryptoKey(): Promise<CryptoKey> {
   if (!cryptoKey) {
      cryptoKey = await crypto.subtle.importKey("raw", keyBuffer, { name: ALGORITHM, length: 256 }, false, ["encrypt", "decrypt"]);
   }
   return cryptoKey;
}
...
export async function encryptKey(plaintext: string): Promise<string> {
   ...
   const encryptedBuffer = await crypto.subtle.encrypt({ name: ALGORITHM, iv }, key, plaintextBytes);

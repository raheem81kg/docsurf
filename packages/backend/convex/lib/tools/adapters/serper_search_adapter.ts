import type { SearchAdapter, SearchAdapterConfig, SearchOptions, SearchResult } from "./search_adapter";
...
export class SerperSearchAdapter implements SearchAdapter {
   ...
   async search(query: string, options: SearchOptions = {}): Promise<SearchResult[]> {
      ...
      try {
         ...
         throw new Error(`Failed to search with Serper: ${error instanceof Error ? error.message : "Unknown error"}`);
      }
   }
         ...
         if (result.content && totalContextUsed < this.MAX_CONTEXT_WINDOW) {
            ...
            if (result.content.length <= remainingContext) {
               ...
               enrichedResults.push({
                  ...
                  markdown: result.markdown ? `${result.markdown.substring(0, remainingContext)}...` : truncatedContent,
               });

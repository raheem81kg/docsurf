...
export const WebSearchAdapter: ToolAdapter = async (params) => {
   ...
   return {
      web_search: tool({
         description: "Search the web for information. Optionally scrape content from results for detailed information.",
         parameters: z.object({
         ...
         execute: async ({ query, scrapeContent }) => {
            ...
            const shouldScrapeContent = scrapeContent ?? userSettings.searchIncludeSourcesByDefault;
            try {
               ...
               console.log(`Searching for ${query} with provider ${userSettings.searchProvider}...`);

   ...
   handler: async (ctx, args) => {
      ...
      for (const [providerId, provider] of Object.entries(args.customProviders)) {
         newSettings.customAIProviders[providerId] = {
            ...
            endpoint: provider.endpoint,
            name: provider.name,
            encryptedKey: provider.newKey ? await encryptKey(provider.newKey) : settings.customAIProviders[providerId].encryptedKey,
         };
      }
      ...
      if (args.generalProviders) {
         for (const [providerId, providerData] of Object.entries(args.generalProviders)) {
            if (providerData) {
               const existingProvider = settings.generalProviders?.[providerId as keyof typeof settings.generalProviders];

               if (providerId === "brave") {
                  newSettings.generalProviders.brave = {
                     enabled: providerData.enabled,
                     encryptedKey: providerData.newKey ? await encryptKey(providerData.newKey) : existingProvider?.encryptedKey || "",
...
export const getSupermemoryKey = internalQuery({
   ...
   handler: async (ctx, args): Promise<string | null> => {
      ...
      if (!settings.generalProviders?.supermemory?.enabled || !settings.generalProviders?.supermemory.encryptedKey) {
         return null;
      }
...
export const getDecryptedGeneralProviderKey = internalQuery({
   ...
   handler: async (ctx, args): Promise<string | null> => {
      ...
      const providerConfig = settings.generalProviders?.[args.providerId as keyof typeof settings.generalProviders];
      ...
      if (args.generalProviderUpdates) {
         ...
         for (const [providerId, update] of Object.entries(args.generalProviderUpdates)) {
            ...
            if (providerId === "brave") {
               newSettings.generalProviders.brave = {
                  enabled: update.enabled,
                  encryptedKey: update.newKey ? await encryptKey(update.newKey) : existingProvider?.encryptedKey || "",

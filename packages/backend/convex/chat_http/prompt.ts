...
export const buildPrompt = (enabledTools: AbilityId[], userSettings?: Infer<typeof UserSettings>) => {
  ...
  - For \`react\`:
   ...
   if (userSettings?.customization) {
      ...
      if (customization.additionalContext) {
         personalizationParts.push(`- Additional context about the user: ${customization.additionalContext}`);
      }

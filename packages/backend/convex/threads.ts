...
import { type FieldPaths, type FilterBuilder, type GenericTableInfo, paginationOptsValidator } from "convex/server";
...
export const createThreadOrInsertMessages = internalMutation({
   ...
   handler: async (ctx, { threadId, authorId, userMessage, proposedNewAssistantId, targetFromMessageId, targetMode, folderId }) => {
      ...
      if (targetFromMessageId) {
         ...
         const targetMessageIndex = allMessages.findIndex((msg) => msg.messageId === targetFromMessageId);
...
const isEmpty = <A extends GenericTableInfo, B extends FieldPaths<A>>(q: FilterBuilder<A>, field: B) =>
   q.or(
   ...
   handler: async ({ db, auth }, { paginationOpts, includeInFolder }) => {
      ...
      if (isFirstPage) {
         ...
         const [pinnedThreads, regularThreadsResult] = await Promise.all([
            !includeInFolder ? pinnedQuery.filter((q) => isEmpty(q, "projectId")).collect() : pinnedQuery.collect(),
            !includeInFolder
...
export const shareThread = action({
   ...
   handler: async (ctx, { threadId, includeAttachments = false }) => {
      ...
      const thread: Infer<typeof Thread> | null = await ctx.runQuery(internal.threads.getThreadById, { threadId });
      if (!thread || thread.authorId !== user.id) {
         return { error: "Unauthorized" };
      }

      // Get all messages for the thread
      const messages: Infer<typeof Message>[] = await ctx.runQuery(internal.messages.getMessagesByThreadId, { threadId });

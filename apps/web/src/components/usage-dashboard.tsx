...
export function UsageDashboard({ className }: UsageDashboardProps) {
   ...
   const tokenTypeData = useMemo(() => {
      ...
      return chartData.map((day) => ({
         ...
         completion: Object.values(day.models).reduce((sum, model) => sum + model.completionTokens, 0),
         reasoning: Object.values(day.models).reduce((sum, model) => sum + model.reasoningTokens, 0),
      }));
   }, [chartData]);
         ...
         <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <Card className="gap-3 p-4">
               <CardHeader className="flex flex-row items-center px-0">
                  ...
                  <CardTitle className="font-medium text-xs sm:text-sm">Total Requests</CardTitle>
               </CardHeader>
               <CardContent className="p-0">
                  <div className="font-bold text-lg sm:text-2xl">{stats?.totalRequests || 0}</div>
                  <p className="text-muted-foreground text-xs">
                     {timeframe === "1d" ? "today" : `last ${timeframe === "7d" ? "7" : "30"} days`}
                  </p>
               </CardContent>
            </Card>
            ...
            <Card className="gap-3 p-4">
               <CardHeader className="flex flex-row items-center px-0">
                  ...
                  <CardTitle className="font-medium text-xs sm:text-sm">Total Tokens</CardTitle>
               </CardHeader>
               <CardContent className="p-0">
                  <div className="font-bold text-lg sm:text-2xl">{((stats?.totalTokens || 0) / 1000).toFixed(1)}K</div>
            ...
            <Card className="gap-3 p-4">
               <CardHeader className="flex flex-row items-center px-0">
                  ...
                  <CardTitle className="font-medium text-xs sm:text-sm">Models Used</CardTitle>
               </CardHeader>
               <CardContent className="p-0">
                  <div className="font-bold text-lg sm:text-2xl">{stats?.modelStats.length || 0}</div>
            ...
            <Card className="gap-3 p-4">
               <CardHeader className="flex flex-row items-center px-0">
                  ...
                  <CardTitle className="font-medium text-xs sm:text-sm">Avg tokens/request</CardTitle>
               </CardHeader>
               <CardContent className="p-0">
         ...
         <div className="grid gap-4">
            ...
            <Card className="gap-3 overflow-hidden p-4">
               ...
               <CardContent className="p-3 px-0">
                  <ChartContainer config={modelChartConfig} className="aspect-auto h-[250px] w-full overflow-hidden sm:h-[300px]">
                     <BarChart
                        data={modelUsageData}
                        width={undefined}
                        height={undefined}
                     ...
                     >
                        ...
                        />
                        <YAxis width={30} tickFormatter={(value) => `${(value / 1000).toFixed(0)}K`} fontSize={isMobile ? 10 : 12} />
                        <ChartTooltip
            ...
            <Card className="gap-3 overflow-hidden p-4">
               ...
               <CardContent className="p-3 px-0">
                  <ChartContainer config={tokenChartConfig} className="aspect-auto h-[250px] w-full overflow-hidden sm:h-[300px]">
                     <BarChart
                        data={tokenTypeData}
                        margin={{ left: 0, right: 10, top: 5, bottom: 0 }}
                        width={undefined}
                     ...
                     >
                        ...
                        <Bar dataKey="completion" stackId="tokens" fill={"var(--chart-2)"} />
         ...
         <Card className="gap-3 p-4">
            ...
            <CardContent className="p-0 pt-0">
               <div className="space-y-1.5">
                  {stats?.modelStats.map((model, index: number) => (
                     <div key={model.modelId} className="flex items-center justify-between rounded-lg border px-2 py-1">
                        ...
                        <div className="text-right">
                           <div className="font-medium text-sm sm:text-base">{(model.totalTokens / 1000).toFixed(1)}K tokens</div>
                           <div className="text-muted-foreground text-xs">
                  ...
                  {(!stats?.modelStats || stats.modelStats.length === 0) && (
                     <div className="py-8 text-center text-muted-foreground">No usage data for the selected period</div>
                  )}
               </div>
            </CardContent>
         </Card>

...
export const ARTIFACT_SUPPORTED_LANGUAGES = ["mermaid", "html", "react", "jsx", "tsx", "markdown", "md"] as const;
   ...
   useEffect(() => {
      (async () => {
         try {
            ...
            const { svg } = await mermaid.default.render(`mermaid-${Date.now()}-${isDark ? "dark" : "light"}`, code);
            console.log(svg);
            setMermaidHTML(svg);
         } catch (err) {
...
export const ArtifactPreview = memo(({ code, language, className }: ArtifactPreviewProps) => {
   const renderPreview = () => {
      switch (language) {
         ...
         default:
            return <div className="p-4 text-center text-muted-foreground">Preview not available for {language}</div>;
      }
   };

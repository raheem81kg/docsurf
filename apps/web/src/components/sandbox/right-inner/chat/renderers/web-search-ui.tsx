...
const FaviconWithLoader = memo(({ url }: { url: string }) => {
   ...
   return (
      <div className="relative flex aspect-square h-4 w-4 items-center justify-center rounded-full">
         {!imageLoaded && <div className="absolute inset-0 animate-pulse bg-muted-foreground/10" />}
         <img
...
export const WebSearchToolRenderer = memo(({ toolInvocation }: { toolInvocation: ToolInvocation }) => {
      ...
      <div className="w-full">
         ...
         >
            ...
            <div className="flex flex-1 items-center gap-2">
               {isLoading ? <Loader2 className="size-4 animate-spin text-primary" /> : <Globe className="size-4 text-primary" />}
               ...
               {!isLoading && hasResults && (
                  <div className={cn("transform transition-transform md:hidden", isExpanded ? "rotate-180" : "")}>
            ...
            {(toolInvocation.args?.query || hasResults) && (
               <div className="flex items-center gap-2 md:ml-auto">
                     ...
                     {hasResults && (
                        <span className="text-muted-foreground text-sm">
                           <div className="flex items-center gap-2">
                              ...
                              <span className="truncate">{toolInvocation.result.results.length} results</span>
                           </div>
                        </span>
                     )}
                  </div>
                  ...
                  {/* Desktop chevron */}
                  {!isLoading && hasResults && (
                     <div className={cn("hidden transform transition-transform md:block", isExpanded ? "rotate-180" : "")}>
                        <ChevronDown className="size-4 text-foreground" />
                     </div>
                  )}
               </div>

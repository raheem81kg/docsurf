import { ImageSkeleton } from "@docsurf/ui/components/image-skeleton";
...
export const ImageGenerationToolRenderer = memo(({ toolInvocation }: { toolInvocation: ToolInvocation }) => {
   ...
   if (isLoading) {
      return (
         <div className="w-full max-w-md overflow-hidden rounded-xl border bg-muted/5" style={{ aspectRatio: cssAspectRatio }}>
            <ImageSkeleton
               rows={rows}
               cols={cols}
               dotSize={3}
   ...
   if (hasError) {
      return (
         ...
         >
            ...
            <p className="text-destructive text-sm">{toolInvocation.result.error || "Failed to generate image"}</p>
         </div>
      );
   }
   ...
   if (hasResult && toolInvocation.result.assets) {
      ...
      return assets.map((asset: any, index: number) => (
         <ImageWithErrorHandler key={index} asset={asset} prompt={prompt} modelName={modelName} cssAspectRatio={cssAspectRatio} />
      ));
   }
...
const ImageWithErrorHandler = memo(
   ({ asset, prompt, modelName, cssAspectRatio }: { asset: any; prompt: string; modelName: string; cssAspectRatio: string }) => {
      ...
      return (
         <div className="not-prose relative w-full max-w-md overflow-hidden" style={{ aspectRatio: cssAspectRatio }}>
            <img
               src={`${env.VITE_CONVEX_URL}/r2?key=${asset.imageUrl}`}

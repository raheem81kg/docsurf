...
export const useVoiceRecorder = ({ onTranscript }: UseVoiceRecorderOptions) => {
   ...
   const verifyToken = useVerifyToken("You must be logged in to use voice input.");
   ...
   const startRecording = useCallback(async () => {
      try {
         // No verifyToken here; just start recording
   ...
   const transcribeAudio = useCallback(
      async (audioBlob: Blob) => {
         try {
            // Refetch token before protected action

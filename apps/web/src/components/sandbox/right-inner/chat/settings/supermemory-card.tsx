...
import type { UserSettings } from "@docsurf/backend/convex/schema/settings";
...
export const SupermemoryCard = memo(({ userSettings, onSave, loading }: SupermemoryCardProps) => {
   ...
   const [enabled, setEnabled] = useState(userSettings.generalProviders?.supermemory?.enabled || false);
               ...
               {isEditing && (
                  <div className="space-y-4">
                     <div className="flex items-center space-x-2">
                        <Switch id="supermemory-enabled" checked={enabled} onCheckedChange={setEnabled} />
                        ...
                        <div className="space-y-3">
                           ...
                           {(!hasExistingKey || rotatingKey) && (
                              <div className="space-y-2">
                                 <Label htmlFor="supermemory-key">{rotatingKey ? "New API Key" : "API Key"}</Label>
                                 <Input
                                    id="supermemory-key"
                                    type="password"
                                    value={newKey}
                           ...
                           {enabled && !hasExistingKey && !newKey.trim() && (
                              <div className="flex items-center gap-2 text-amber-600">
                                 ...
                                 <span className="text-sm">API key required to enable Supermemory</span>
                              </div>
                           )}
                        </div>
                     )}

...
import { EmailVerificationTemplate, OTPEmailTemplate, PasswordResetTemplate } from "./email-templates";

// Email provider types
type EmailProvider = "resend" | "local-only-mock";
...
class EmailService {
      ...
      try {
         ...
         if (this.config.provider === "local-only-mock") {
            if (process.env.NODE_ENV !== "development") {
               throw new Error("Local mock email provider is only available in development mode");
            }
   ...
   async sendVerificationEmail(data: { user: { email: string; name?: string }; url: string; token: string }) {
   ...
   async sendPasswordResetEmail(data: { user: { email: string; name?: string }; url: string; token: string }) {
      const html = await render(
   ...
   async sendOTPEmail(data: { email: string; otp: string; type: "sign-in" | "email-verification" | "forget-password" }) {
      const getSubjectAndTemplate = async () => {
         switch (data.type) {
            case "sign-in":
               return {
                  subject: "Your sign-in code - Docsurf",
                  html: await render(
                     OTPEmailTemplate({
                        otp: data.otp,
                        type: "sign-in",
                  ...
                  text: `Your sign-in code for Docsurf is: ${data.otp}\n\nThis code will expire in 5 minutes.`,
               };
            case "email-verification":
               return {
                  subject: "Verify your email - Docsurf",
                  html: await render(
                     OTPEmailTemplate({
                        otp: data.otp,
                  ...
                  text: `Your email verification code for Docsurf is: ${data.otp}\n\nThis code will expire in 5 minutes.`,
               };
            case "forget-password":
               return {
                  ...
                  text: `Your password reset code for Docsurf is: ${data.otp}\n\nThis code will expire in 5 minutes.`,
               };
         }
      };

...
import { MODELS_SHARED, type SharedModel, type CoreProvider } from "@docsurf/backend/convex/lib/models";
import type { ModelAbility, UserSettings } from "@docsurf/backend/convex/schema/settings";
...
export function useAvailableModels(userSettings: Infer<typeof UserSettings> | undefined) {
   ...
   Object.entries(userSettings?.customModels || {}).forEach(([id, customModel]) => {
      ...
      const hasProvider =
         currentProviders.core[customModel.providerId]?.enabled || currentProviders.custom[customModel.providerId]?.enabled;

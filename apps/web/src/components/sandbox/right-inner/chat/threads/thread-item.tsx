...
export const ThreadItem = memo(
   ({ thread, isInFolder = false, onOpenRenameDialog, onOpenMoveDialog, onOpenDeleteDialog }: ThreadItemProps) => {
      ...
      return (
         <SidebarMenuItem className={isInFolder ? "pl-6" : ""}>
            ...
            >
               <SidebarMenuButton asChild className={cn("flex-1 hover:bg-transparent", isActive && "text-foreground")}>
                  <Link to="/thread/$threadId" params={{ threadId: thread._id }} className="flex items-center justify-between">
                     ...
                     <DropdownMenu onOpenChange={setIsMenuOpen}>
                        <DropdownMenuTrigger asChild>
                           <button
                              type="button"
                              className={cn("rounded p-1 transition-opacity", isMenuOpen || "opacity-0 group-hover/item:opacity-100")}
                           >
   ...
   (prevProps, nextProps) => {
      return equal(prevProps.thread, nextProps.thread) && prevProps.isInFolder === nextProps.isInFolder;
   }
);

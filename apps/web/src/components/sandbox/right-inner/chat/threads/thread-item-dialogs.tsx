...
import { Dialog, DialogContent, DialogFooter, DialogHeader, DialogTitle } from "@docsurf/ui/components/dialog";
...
import { getProjectColorClasses } from "@/components/sandbox/right-inner/chat/lib/project-constants";
...
export const ThreadItemDialogs = memo(
   ({
      ...
      const handleRename = async () => {
         ...
         try {
            ...
            if (result && "error" in result) {
               toast.error(typeof result.error === "string" ? result.error : "Failed to rename thread");
            } else {
      ...
      const handleMove = async () => {
         ...
         const newProjectId = selectedProjectId === "no-folder" ? undefined : (selectedProjectId as Id<"projects">);
         ...
         try {
            ...
            if (result && "error" in result) {
               toast.error(typeof result.error === "string" ? result.error : "Failed to move thread");
            } else {
               const targetName = newProjectId ? projects.find((p) => p._id === newProjectId)?.name || "folder" : "General";
         ...
         <>
            ...
            >
               <DialogContent>
                  ...
                  <DialogFooter>
                     <Button variant="outline" onClick={onCloseRenameDialog} disabled={isRenaming}>
                        Cancel
                     </Button>
            ...
            >
               <DialogContent>
                  ...
                  <div className="space-y-4">
                     <RadioGroup value={selectedProjectId} onValueChange={setSelectedProjectId} disabled={isMoving}>
                        {/* No folder option */}
                        <div className="flex items-center space-x-2">
                           <RadioGroupItem value="no-folder" id="no-folder" />
                           <Label htmlFor="no-folder" className="flex cursor-pointer items-center gap-2">
                              <span>No Folder</span>
                           </Label>
                        </div>
                        ...
                        {projects.map((project) => {
                           ...
                           return (
                              <div key={project._id} className="flex items-center space-x-2">
                                 ...
                                 <Label htmlFor={project._id} className="flex cursor-pointer items-center gap-1">
                                    <div
                                       className={cn(
                                    ...
                                    >
                                       <FolderOpen className="size-4" fill="currentColor" strokeWidth={1} stroke="var(--foreground)" />
                                    </div>
                  ...
                  <DialogFooter>
                     <Button variant="outline" onClick={onCloseMoveDialog} disabled={isMoving}>
                        Cancel
                     </Button>
                     <Button
                        onClick={handleMove}
                        disabled={isMoving || selectedProjectId === (currentThread?.projectId || "no-folder")}
                     >
                        {isMoving ? (
                           <>
                              <Loader2 className="h-4 w-4 animate-spin" />
            ...
            >
               <AlertDialogContent>
                  <AlertDialogHeader>
                     ...
                     <AlertDialogDescription>
                        Are you sure you want to delete <span className="font-bold">{currentThread?.title?.trim()}</span>? <br /> This

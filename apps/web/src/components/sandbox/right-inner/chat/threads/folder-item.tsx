...
} from "@/components/sandbox/right-inner/chat/lib/project-constants";
...
export function FolderItem({ project, numThreads }: { project: Project; numThreads: number }) {
   ...
   const handleEdit = async () => {
      ...
      try {
         ...
         if (result && "error" in result) {
            toast.error(typeof result.error === "string" ? result.error : "Failed to update folder");
         } else {
   ...
   const handleDelete = async () => {
      ...
      try {
         ...
         if (result && "error" in result) {
            toast.error(typeof result.error === "string" ? result.error : "Failed to delete folder");
   ...
   return (
      <>
         <SidebarMenuItem>
            ...
            >
               <SidebarMenuButton asChild className={cn("flex-1 hover:bg-transparent", isCurrentFolder && "text-foreground")}>
                  <Link
                     onClick={() => {
                        setOpenMobile(false);
                     }}
                  ...
                  <AlertDialogDescription>
                     ...
                     {numThreads > 0 && (
                        <>
                           ...
                           {numThreads !== 1 ? "s" : ""}. The folder will be archived instead of deleted.
                        </>
                     )}
                  </AlertDialogDescription>
               </AlertDialogHeader>

...
function InfoCard({ children, className, storageKey, dismissType = "once", forceDismiss, onDismissed }: InfoCardProps) {
   ...
   useEffect(() => {
      if (forceDismiss && !isDismissed) {
         setIsDismissed(true);
         if (dismissType === "forever" && storageKey) {
            localStorage.setItem(storageKey, "dismissed");
         }
      }
   }, [forceDismiss, isDismissed, dismissType, storageKey]);
...
const InfoCardMedia = ({ media = [], className, loading = undefined, shrinkHeight = 75, expandHeight = 150 }: InfoCardMediaProps) => {
   ...
   return (
      ...
      >
         <motion.div
            className={cn("relative mt-2 rounded-sm", className)}
            animate={{
                  ...
                  return (
                     ...
                     >
                        {type === "video" ? (
                           <video
                              src={src}
                              className={cn("w-full rounded-sm border border-gray-200 object-cover shadow-lg", itemClassName)}

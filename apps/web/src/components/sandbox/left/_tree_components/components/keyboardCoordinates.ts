...
export const sortableTreeKeyboardCoordinates: (
   ...
   (event, { currentCoordinates, context: { active, over, collisionRect, droppableRects, droppableContainers } }) => {
      // Helper to determine if an item is a folder
      const isFolder = (id: UniqueIdentifier) => {
         const item = context.current.items.find((item) => item.id === id);
      ...
      if (directions.includes(event.code)) {
         ...
         if (horizontal.includes(event.code) && over?.id) {
            const { depth, maxDepth, minDepth } = getProjection(items, active.id, over.id, offset, indentationWidth, isFolder);
         ...
         if (closestId === over?.id && collisions.length > 1 && collisions[1]?.id !== undefined) {

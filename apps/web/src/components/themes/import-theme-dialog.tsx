...
import { Form, FormControl, FormField, FormItem, FormMessage } from "@docsurf/ui/components/form";
import { Input } from "@docsurf/ui/components/input";
import { fetchThemeFromUrl, type ThemePreset } from "../sandbox/right-inner/chat/lib/theme-utils";

const formSchema = z.object({
   url: z.string().min(1, { message: "Theme URL is required" }).url({ message: "Please enter a valid URL" }),
});
               ...
               <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  {fetchAndApplyThemeMutation.error && (
                     <div className="flex items-start gap-2 rounded-md border border-destructive/20 bg-destructive/10 p-3">
                        ...
                        <p className="text-destructive text-sm leading-relaxed">{fetchAndApplyThemeMutation.error.message}</p>
                     </div>
                  )}

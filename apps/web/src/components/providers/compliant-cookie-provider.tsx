...
import CookieUtils from "@/lib/cookies/cookie-utils";
...
export function CompliantCookieProvider({ children }: { children: React.ReactNode }) {
   ...
   const acceptAll = () => {
      const newPreferences = Object.keys(COOKIE_CATEGORIES).reduce((acc, key) => ({ ...acc, [key]: true }), {});
      setPreferences(newPreferences);
      savePreferences(newPreferences);
   };
   ...
   const hasConsent = useMemo(() => Object.values(preferences).some((value) => value), [preferences]);

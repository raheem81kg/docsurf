...
import type { MinimalTiptapProps } from "../custom/minimal-tiptap";
...
/**
 * Extracts HTML for a block node using ProseMirror's DOMSerializer.
 * Optimized to minimize DOM operations and reuse serializer.
 */
export function getBlockHtml(editor: Editor, node: ProseMirrorNode, pos: number): string {
   if (!editor?.state?.doc || !node) return "";
   ...
   try {
      ...
      // Create serializer once and reuse
      ...
      console.error("Error getting block HTML:", error);
      return "";
   }
}
